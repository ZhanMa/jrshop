...
class DefaultPassportUser extends BasePassportUser
{
    ...
    function edit($user_id, $old_password, $items, $force = false)
    {
        ...
        if (isset($items['safephone']))
        {
        	$edit_data['safephone'] = $items['safephone'];
        }
        if (empty($edit_data))
        {
            return false;
    ...
    function check_phone($user_id,$phonenum)
    {
    	$model_member =& m('member');
    	$info = $model_member->get("safephone='{$phonenum}' AND user_id!='{$user_id}'");
    	if (!empty($info))
    	{
