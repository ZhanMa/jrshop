...
class FrontendApp extends ECBaseApp
{
    ...
    function display($tpl)
    {
		...
		$carts_top = $this->_get_carts_top(SESS_ID, $this->visitor->get('user_id'));
		...
		$this->assign('store_site_logo', Conf::get('store_site_logo'));
        $this->assign('statistics_code', Conf::get('statistics_code')); // 统计代码
		//echo Conf::get('statistics_code');
        $current_url = explode('/', $_SERVER['REQUEST_URI']);
        $count = count($current_url);
		$url_info=$_SERVER['QUERY_STRING'];
		$this->assign('url_info',$url_info);
		/*分类 by ce*/
		$this->assign('mainnav',$this->mainnav());
		$this->assign('all_gcategory',$this->_get_all_gcategory());
		/* 热门搜素 */
	...
	function mainnav()
	{
		...
		$gcategory = $tree->getArrayList(0);	
		$brand_mod=&m('brand');
		$brand=$brand_mod->find(array('conditions'=>'recommended=1'));
		$data=array('gcategory'=>$gcategory,'brand'=>$brand);
...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user','check_phone','get_code')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
...
class StorebaseApp extends FrontendApp
{
    ...
    function get_store_data()
    {
        ...
        if ($store === false)
        {
			...
			/* 增加店铺商品热销排行榜 by tyioocom*/
			$store['hot_saleslist'] = $this->_get_hot_saleslist();
			/* end */
			/* 增加店铺商品收藏排行榜 by tyioocom*/
			...
			// 将热门搜索，在线客服，店铺幻灯片 转为数组 tyioocom
