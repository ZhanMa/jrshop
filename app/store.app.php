...
class StoreApp extends StorebaseApp
{
    function index()
    {
		...
		$this->assign('rank',$this->rank($id));
        $this->display('store.index.html');
    }
	function rank($id){
		$gstatic_mod=&m('goodsstatistics');
		$goods_mod=&m("goods");
		$rank=$goods_mod->getAll("SELECT * FROM {$goods_mod->table} g JOIN {$gstatic_mod->table} gs ON g.goods_id=gs.goods_id WHERE g.store_id='".$id."' ORDER BY gs.sales DESC");
		return $rank;
	}
	/* 取得浏览历史 */
	function _huoban($id)
    ...
    function search()
    {
		...
		$this->assign('goods_history', $this->_get_goods_history());
		$this->assign('rank',$this->rank($id));
		$this->assign('partners',$this->_huoban($id));
    ...
    function article()
    {
		...
		$this->assign('goods_history', $this->_get_goods_history());
		$this->assign('rank',$this->rank($article['store_id']));
		$this->assign('partners',$this->_huoban($article['store_id']));
    ...
    function credit()
    {
		...
		$this->assign('goods_history', $this->_get_goods_history());
		$this->assign('rank',$this->rank($store['store_id']));
		$this->assign('partners',$this->_huoban($store['store_id']));
    ...
    function _assign_searched_goods($id)
    {
		...
		if($mnprice['min'] && $mnprice['min']>0){
			$conditions.=" AND g.price>".$mnprice['min'];
		}
		if($mnprice['max'] && $mnprice['max']>0){
			$conditions.=" AND g.price<".$mnprice['max'];
		}
        if ($conditions)
        {
			if($_GET['keyword']){
			...
			if($mnprice['min']){
				$search_name.= sprintf(LANG::get('price_include'),$mnprice['min'],$mnprice['max']);
			}
            $sgcate_id   = 0;
        }
        else
