...
define('PRICE_INTERVAL_NUM', 20);   // 价格区间个数
define('MIN_STAT_STEP', 5);       // 价格区间最小间隔
define('NUM_PER_PAGE', 1);        // 每页显示数量
...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		//print_r($this->_get_filter($param));

        /* 按分类、品牌、地区、价格区间统计商品数量 */
        $stats = $this->_get_group_by_info($param, ENABLE_SEARCH_CACHE);
		foreach($stats['by_price'] as $k=>$v){
			$ppara=explode('-',$_GET['price']);
			$stats['by_price'][$k]['compare']=$ppara[0];
			$stats['by_price'][$k]['pc']=$ppara[1];	
		}
        ...
        /* 分页信息 */
		$numper=ecm_getcookie('numper')?ecm_getcookie('numper'):20;
        $page = $this->_get_page($numper);
        ...
        foreach ($goods_list as $key => $goods)
        {
			...
			$goods_list[$key]['default_image']=$goods_image['thumbnail'];
			$goods_list[$key]['big_image']=$goods_image['image_url'];
            empty($goods['default_image']) && $goods_list[$key]['default_image'] = Conf::get('default_goods_image');
            $goods_list[$key]['grade_name'] = $sgrades[$goods['sgrade']];
			$add_time=$goods['add_time'];
			$count_views+=$goods['views'];
			
			$interval=(gmtime()-$add_time)/3600;
			if($interval<48){
				$goods_list[$key]['new']=1;	
			}else{
				$goods_list[$key]['new']=0;	
			}
			
			/* 如果库存小于5件，则显示清仓 */
			if($goods['stock']<5)
			{
				$goods_list[$key]['new']=2;	
			}
        }	
		if($goods_list && $count_views){
			$avg=$count_views/count($goods_list);
		};
		$this->assign('avg',$avg);
        ...
        if (empty($display_mode) || !in_array($display_mode, array('list_1', 'squares_1')))
        {
            $display_mode = 'squares_1'; // 默认格子方式
        }
        $this->assign('display_mode', $display_mode);
		$this->assign('numper',$numper);
        /* 取得导航 */
    ...
    function store()
    {
		...
		if($stores){
			foreach ($stores as $k=>$v){
				$stores[$k]['goods_list']=$goods_mod->find(array(
					'conditions'=>'store_id='.$v['store_id'],
					'order'     =>'add_time desc',
					'limit'     =>'12'
				));
			}
		}
		$goods_item=$goods_mod->find();
		$count=count($goods_item);
		$count=str_split($count);
		$store_mod=&m('store');
		$re_store=$store_mod->find(array(
			'conditions'=>'recommended=1',
			'limit'     =>10
		));
		 foreach ($re_store as $key => $store)
        {
			//等级图片
            $step = intval(Conf::get('upgrade_required'));
            $step < 1 && $step = 5;
            $re_store[$key]['credit_image'] = $this->_view->res_base . '/images/' . $model_store->compute_credit($store['credit_value'], $step);
		}
		$this->assign('re_store',$re_store);
		 $this->assign('count',$count);
		 $this->assign('store_num', str_split(count($store_mod->find())));
    ...
    function groupbuy()
    {
        ...
        foreach ($groupbuy_list as $key => $groupbuy)
        {
			...
			$price_array=explode('.', $groupbuy_list[$key]['group_price']);
			$groupbuy_list[$key]['group_price']=$price_array[0];
			$groupbuy_list[$key]['group_price_left']=$price_array[1];
            $groupbuy['state'] == GROUP_ON && $groupbuy_list[$key]['lefttime'] = lefttime($groupbuy['end_time']);
			$groupbuy_list[$key]['discount']=number_format(($groupbuy_list[$key]['group_price']/$groupbuy['price'])*10,1);
 ;
			$start_time=$groupbuy['start_time'];
			$interval=(mktime()-$start_time)/3600;
			if($interval<48){
				$groupbuy_list[$key]['new']=1;	
			}else{
				$groupbuy_list[$key]['new']=0;
			}
        }
