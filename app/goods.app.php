...
class GoodsApp extends StorebaseApp
{
    ...
    function index()
    {
		...
		$this->assign('store',$this->store11());
        $this->display('goods.index.html');
    }
	function _get_partners($id)
    {
		$id=$_GET['id'];
		$gstatic_mod=&m('goodsstatistics');
		$goods_mod=&m("goods");
		$store_id=$goods_mod->get(array(
			'conditions'=>'goods_id='.$id,
			'fields'    =>'store_id',
		));
        $partner_mod =& m('partner');
        $partner=$partner_mod->find(array(
            'conditions' => "store_id =".$store_id['store_id'],
            'order' => 'sort_order',
        ));
		foreach($partner as $k=>$v){
			empty($v['logo'])&&$partner[$k]['logo']="http://t2.baidu.com/it/u=3775862801,2844097047&fm=52&gp=0.jpg";
		}
		return $partner;
    }
	function rank(){
		...
		//print_r($store_id);
		$rank=$goods_mod->getAll("SELECT * FROM {$goods_mod->table} g JOIN {$gstatic_mod->table} gs ON g.goods_id=gs.goods_id WHERE g.store_id='".$store_id['store_id']."' ORDER BY gs.sales DESC");
		//print_r($rank);
		return $rank;
	}
	...
	function  store11(){
		...
		$this->set_store($store_id['store_id']);
        $store = $this->get_store_data();
		$qq=explode(" ",$store['im_qq']);
		$ww=explode(" ",$store['im_ww']);
		$qq_name=explode(" ",$store['im_qq_name']);
		$ww_name=explode(" ",$store['im_ww_name']);
		for($i=0;$i<count($qq);$i++){
			$store['qq'][$i]=array($qq[$i],$qq_name[$i]);	
		}
		for($i=0;$i<count($ww);$i++){
			$store['ww'][$i]=array($ww[$i],$ww_name[$i]);	
		}
        return $store;
	}
    function comments()
    ...
    function qa()
    {
        ...
        else
        {
            ...
            {
                header("Location: index.php?app=goods&id={$id}#module\n");
                exit;
            }
            else
            {
    ...
    function _get_goods_comment($goods_id, $num_per_page)
    {
	    ...
	    function _get_goods_comments($goods_id)
    {
        $data = array();
        $order_goods_mod =& m('ordergoods');
		...
		$data['num']=$order_goods_mod->getCount();
        $data['comments'] = $comments;
