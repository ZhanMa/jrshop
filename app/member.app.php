...
class MemberApp extends MemberbaseApp
{
    ...
    function phone(){
    	...
    	if (!IS_POST){
    		...
    		$this->_config_seo('title', Lang::get('user_center') . ' - ' . Lang::get('edit_email'));
    		$ms =& ms();
    		$usermod = $ms->user->get($user_id);
    		if($usermod['safephone']){
    			$safephone = substr($usermod['safephone'],0,3).'****'.substr($usermod['safephone'],6);
    		}else{
    			$safephone = '';
    		}
    		$this->assign('safephone',$safephone);
    		$this->display('member.phone.html');
    	}else{
    		$orig_password  = $_POST['orig_password'];
    		$safephone          = isset($_POST['safephone']) ? trim($_POST['safephone']) : '';
    		if (!$safephone)
    		{
    		...
    		$ms =& ms();    //连接用户系统
    		if(!$ms->user->check_phone($this->visitor->get('user_id'),$safephone)){
    			$this->show_warning('phone_required');
    ...
    function check_phone(){
    	$user_id = $this->visitor->get('user_id');
		$phonenum = empty($_GET['safephone']) ? null : trim($_GET['safephone']);
    	if (!$phonenum)
    	{
    		echo ecm_json_encode(false);
    		return;
    	...
    	echo ecm_json_encode($ms->user->check_phone($user_id,$phonenum));
    }
    ...
    function get_code()
    {
    	...
    	$content = '【96018】欢迎您注册金融街生活在线，请输入以下验证码完成注册，您的验证码为'.$_SESSION['yanzhengma'].'，请您在十分钟内进行提交，您正在用手机绑定您的账户，为了保护您的账号安全，请勿泄露。【96018金融街生活在线】';
    ...
    function check_code()
    {
    	$getyanzhengma = empty($_GET['phonecode']) ? null : trim($_GET['phonecode']);
